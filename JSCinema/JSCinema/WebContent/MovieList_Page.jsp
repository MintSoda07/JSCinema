...
<script>
	...
	function move_to_info(clicked_id) {
		var url="movie_showPage.jsp?img="+clicked_id;
		if(is_remove_enabled){
			var input = confirm('삭제하시겠습니까?')
			if(input==true){
				location.href=("movie_RemoveRedirection.jsp?img="+clicked_id);	
			}else{
				return;
			}
			...
			if(input==true){
				location.href=("movie_editPage.jsp?img="+clicked_id);	
			}else{
				return;
			}
		}else{
	...
	function add() {
		...
		location.href="movie_addPage.jsp";
	}
...
<div style="font-size:40px; color:white; "><span style="margin-left:30px;">영화</span><hr></div>
<div class="list_title" style="background:black;">
<%
try{
	
	if(session.getAttribute("Manager").equals("Y")){// 삭제, 관리기능 추가요망
		out.print("<div id=\"item_remove\">항목 삭제</div><div id=\"item_edit\">영화 관리</div><div id=\"item_add\" onclick=\"add()\">항목 추가</div>");
		}
}catch(Exception e){
...
try {
 ...
 String sql = "SELECT NAME,INFOMATION,OPEN_DATE,SIMPLE_INFO,IMG_PATH FROM MOVIE ORDER BY NAME";
